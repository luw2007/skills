...
- **本地只持有 MR 部分 commit 时，`git diff backup HEAD` 必然大且非空 ≠ 丢失**：squash body 列了 N 个 subject，本地却只有其中 M 个（其余在别的机器/review 时补的），backup 是「旧 base + M 个 commit」的子集，本就不该等于 origin/master。此时别套「diff 为空」黄金法则，改用：①`HEAD == origin/master`（`rev-list --left-right --count` 为 `0 0`）；②对本地 commit 改过的文件逐个 `git diff --quiet backup HEAD -- <f>`，差异必须**仅落在 MR 其它 commit 也改过的共享文件**上，独占文件须逐字节相同。两者齐备即零丢失。
